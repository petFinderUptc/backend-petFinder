 ...
 * TODO: FASE 2 - Agregar @UseGuards(JwtAuthGuard) a endpoints protegidos
 */
...
export class PostsController {
  ...
  async create(@Body() createPostDto: CreatePostDto): Promise<IPost> {
   ...
   */
  @Get()
  async findAll(@Query() filters: FilterPostDto): Promise<IPost[]> {
   ...
   */
  @Put(':id/resolve')
  async markAsResolved(@Param('id') id: string): Promise<IPost> {
    // TODO: Obtener userId real desde JWT
    const mockUserId = 'current-user-id';
  ...
  // @Get(':id/matches')
  // async findMatches(@Param('id') id: string): Promise<IPost[]>

  // TODO: FASE 2 - Agregar endpoint de búsqueda geoespacial
  // @Get('nearby')
  // async findNearby(@Query() location: LocationQueryDto): Promise<IPost[]>
}
