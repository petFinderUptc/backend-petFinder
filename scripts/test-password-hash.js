...
async function testPasswordHashing() {
  ...
  console.log(`¿Todos válidos para la misma contraseña?: ${(valid1 && valid2 && valid3) ? '✅ SÍ' : '❌ NO'}`);
  ...
  for (const rounds of roundsToTest) {
    ...
    const recommendation = rounds === 12 ? '✅ Recomendado' : 
                          rounds < 10 ? '⚠️ Bajo' : 
                          rounds > 12 ? '🔒 Alto' : '';
    
    console.log(`| ${rounds.toString().padEnd(6)} | ${time.toString().padEnd(11)} | ${hashesPerSec.toString().padEnd(10)} | ${recommendation.padEnd(13)} |`);
  }
  ...
  invalidHashes.forEach(hash => {
    ...
    console.log(`  "${hash.substring(0, 30)}${hash.length > 30 ? '...' : ''}".padEnd(35)} → ${isValid ? '✅ Válido' : '❌ Inválido'}`);
  });
